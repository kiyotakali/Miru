...
def api_shutdown():
    ...
    def _do_shutdown():
        ...
        linux_desktop = sys.platform.startswith("linux") and os.environ.get("MIRU_DESKTOP_PLATFORM") == "linux"
...
def _launch_pet_tauri(pet_url, pet_hotkey, env, cwd):
    ...
    if sys.platform.startswith("linux") and pet_env.get("MIRU_DESKTOP_PLATFORM") == "linux":
...
def _respawn_pet():
    ...
    def _do():
        ...
        # Write PID file
        try:
            if not (sys.platform.startswith("linux") and os.environ.get("MIRU_DESKTOP_PLATFORM") == "linux"):
...
def _launch_full_stack():
    ...
    env = os.environ.copy()
    linux_desktop = sys.platform.startswith("linux") and env.get("MIRU_DESKTOP_PLATFORM") == "linux"
    ...
    def _start_pet():
        ...
        if pet_pid_file.exists() and not linux_desktop:
            try:
