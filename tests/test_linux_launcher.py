...
def _pet_launcher(platform):
    # Exercise subprocess dispatch without starting the Flask/model runtime.
    import os
    source = Path(__file__).resolve().parents[1] / "app.py"
    tree = ast.parse(source.read_text(encoding="utf-8"))
    function = next(node for node in tree.body if isinstance(node, ast.FunctionDef)
                    and node.name == "_launch_pet_tauri")
...
def test_pet_dispatch_preserves_auth_and_non_linux_command(monkeypatch, capsys, platform):
    import subprocess
    namespace = _pet_launcher(platform)
    calls = []
...
def test_linux_shutdown_ignores_stale_pid_files(monkeypatch, tmp_path):
    ...
    tree = ast.parse(source.read_text(encoding="utf-8"))
    function = next(node for node in tree.body if isinstance(node, ast.FunctionDef)
                    and node.name == "_kill_pet")
    namespace = {"os": os, "sys": SimpleNamespace(platform="linux"),
                 "_pet_process": None, "_pet_pid_dir": lambda: tmp_path}
    exec(compile(ast.Module(body=[function], type_ignores=[]), str(source), "exec"), namespace)
...
def test_linux_exit_api_stops_owned_children_without_signalling_parent(monkeypatch, tmp_path):
    ...
    namespace = {"os": os, "sys": SimpleNamespace(platform="linux"),
                 "_kill_pet": lambda: calls.append("pet"), "_pet_pid_dir": lambda: tmp_path,
                 "_pet_children_list": [Child()], "jsonify": lambda data: data}
    exec(compile(ast.Module(body=[function], type_ignores=[]), str(source), "exec"), namespace)
    monkeypatch.setenv("MIRU_DESKTOP_PLATFORM", "linux")
