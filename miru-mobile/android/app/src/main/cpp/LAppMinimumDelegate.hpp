...
public:
    ...
    */
    ...
    /** Upload chat bubble texture from Java-rendered RGBA pixels. */
    void SetBubbleTexture(const unsigned char* rgba, int w, int h) {
        _effects.SetBubbleTexture(rgba, w, h);
    }

    /** Clear bubble display. */
    void ClearBubble() {
        _effects.ClearBubble();
    }

    /** Set Miru emotion state for visual effects. */
    void SetEmotionState(float valence, float arousal) {
        _effects.SetEmotionState(valence, arousal);
    }
...
private:
    ...
    */
    ...
    WallpaperEffects _effects;                   ///< Background gradient + particles
};
